...
```{r parallel-bootstrap, cache = TRUE}
library(boot)
f <- function(df, i) {
  df.sampled <- df[i,]
...
benchmark(replications=10,
  boot(cars, f, R=1000, parallel="multicore"),
...
```{r, echo=FALSE, results='asis'}
readtime <- read.csv("../readtime.csv")
rownames(readtime) <- readtime[,1]
readtime <- readtime[,-1]
colnames(readtime) <- local({
  tmp <- colnames(readtime)
  sprintf("%s台機器", substring(tmp, 2, nchar(tmp)))
})
rownames(readtime) <- local({
  tmp <- rownames(readtime)
  sprintf("%s倍資料", tmp)
})
print(xtable(readtime), type="html")
```
